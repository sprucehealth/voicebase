...
if [[ "$DEPLOY_TO_S3" != "" ]]; then
    SVCS="auth baymaxgraphql carefinder curbside directory excomms invite notification regimensapi restapi routing threading settings operational deploy layout care"
    for SVC in $SVCS; do
        echo "BUILDING ($SVC)"
