...
// where any elligible doctor can access an item for a period of time. The reason for
// accessing this data using a manager versus directly from the DataAPI is to synchronize on the methods
// so as to ensure that we provide a logically consistent view of the world on accessing items from the global
// case queue, or when modifying access to a case so as to ensure that only 1 doctor is granted access to act
// on a patient case at any given time.
type JBCQManager struct {
	dataAPI DataAPI
	qMutex  sync.Mutex
}

func NewJBCQManager(dataAPI DataAPI) *JBCQManager {
	return &JBCQManager{
		dataAPI: dataAPI,
		qMutex:  sync.Mutex{},
	}
