...
type Suggestion struct {
	...
	InternalName string `json:"internal_id"`
}
...
func (s *AutocompleteHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for i, searchResult := range searchResults {
		...
		if openBracket != -1 {
			...
			autocompleteResponse.Suggestions[i] = Suggestion{Title: searchResult[:openBracket], Subtitle: SpecialTitle(subtitle), InternalName: searchResult}
		} else {
