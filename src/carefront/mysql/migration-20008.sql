...
create table dr_treatment_template_drug_db_id (
	...
	primary key(id),
	foreign key (dr_treatment_template_id) references dr_treatment_template(id)
) character set utf8;

insert into dr_treatment_template_drug_db_id (drug_db_id_tag, drug_db_id, dr_treatment_template_id)
	select drug_db_id.drug_db_id_tag, drug_db_id.drug_db_id, dr_treatment_template.id from drug_db_id 
		inner join treatment on drug_db_id.treatment_id = treatment.id
		inner join dr_treatment_template on dr_treatment_template.treatment_id = treatment.id
		where treatment_plan_id is null;

update dr_treatment_template 
	inner join treatment on dr_treatment_template.treatment_id = treatment.id
...
delete d.* from drug_db_id d 
	inner join treatment on treatment.id = d.treatment_id
	where treatment.treatment_plan_id is null;
...
rename table drug_db_id to treatment_drug_db_id;
