...
func TestFavoriteTreatmentPlan(t *testing.T) {
	...
	if len(responseData.FavoriteTreatmentPlans) != 2 {
		t.Fatalf("Expected 2 favorite treatment plans instead got %d", len(responseData.FavoriteTreatmentPlans))
	}

	if len(responseData.FavoriteTreatmentPlans[0].RegimenPlan.RegimenSections) != 1 {
		t.Fatalf("Expected favorite treatment plan to have 1 regimen section")
	}

	if len(responseData.FavoriteTreatmentPlans[0].Advice.SelectedAdvicePoints) != 1 {
		t.Fatalf("Expected favorite treatment plan to have 1 advice point")
	}

	if len(responseData.FavoriteTreatmentPlans[1].RegimenPlan.RegimenSections) != 2 {
		t.Fatalf("Expected favorite treatment plan to have 2 regimen sections")
	}

	if len(responseData.FavoriteTreatmentPlans[1].Advice.SelectedAdvicePoints) != 2 {
