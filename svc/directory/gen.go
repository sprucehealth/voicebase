...
//go:generate protoc --gogoslick_out=plugins=grpc:. --proto_path=$GOPATH/src:. svc.proto events.proto
...
func init() {
	grpcmetrics.WrapMethods(_Directory_serviceDesc.Methods)
}

func InitMetrics(srv interface{}, mr metrics.Registry) {
	grpcmetrics.InitMetrics(srv, mr, _Directory_serviceDesc.Methods)
}
