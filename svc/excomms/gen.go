...
//go:generate sed -i "" s#golang.org/x/net/context#context#g ./svc.pb.go ./events.pb.go
//go:generate gofmt -w ./svc.pb.go ./events.pb.go

func init() {
	grpcmetrics.WrapMethods(_ExComms_serviceDesc.Methods)
}

func InitMetrics(srv interface{}, mr metrics.Registry) {
	grpcmetrics.InitMetrics(srv, mr, _ExComms_serviceDesc.Methods)
}
