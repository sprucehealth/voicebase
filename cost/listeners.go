...
func InitListeners(dataAPI api.DataAPI, dispatcher *dispatch.Dispatcher) {
	...
	dispatcher.Subscribe(func(ev *VisitChargedEvent) error {
		...
		go func() {
			...
			referralTrackingEntry, err := dataAPI.PendingReferralTrackingForPatient(ev.PatientID)
			...
			if err := referralProgram.Data.(promotions.ReferralProgram).
				ReferredPatientSubmittedVisit(ev.PatientID, referralTrackingEntry.CodeID, dataAPI); err != nil {
				golog.Errorf(err.Error())
				return
			}
		}()
