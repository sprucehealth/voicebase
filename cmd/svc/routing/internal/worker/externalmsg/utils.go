...
// patientEntitiesWithVerifiedContact returns a list of patient entities that have the provided contactValue as a verified contact and
// are part of the provided organization
func patientEntitiesWithVerifiedContact(contactValue string, res *directory.LookupEntitiesByContactResponse, organizationID string) []*directory.Entity {
	if res == nil {
		return nil
	}

	patientEntities := make([]*directory.Entity, 0, len(res.Entities))
	for _, entity := range res.Entities {
		if entity.Type != directory.EntityType_PATIENT {
			continue
		}

		// ensure that the patient is part of the org
		var isMemberOfOrg bool
		for _, m := range entity.Memberships {
			if m.Type == directory.EntityType_ORGANIZATION && m.ID == organizationID {
				isMemberOfOrg = true
				break
		...
		for _, contact := range entity.Contacts {
			if contact.Value == contactValue && contact.Verified {
				patientEntities = append(patientEntities, entity)
				break
			}
		}
	}
