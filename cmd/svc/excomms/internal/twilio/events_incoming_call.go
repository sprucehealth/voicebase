...
func voicemailTWIML(ctx context.Context, params *rawmsg.TwilioParams, eh *eventsHandler) (string, error) {
	...
	if err != nil {
		golog.Errorf("unable to get settings value for orgID %s", orgID)
	} else if len(valueRes.Values) != 2 {
		golog.Errorf("expected 2 values to be returned when querying for settings for %s but got %d", orgID, len(valueRes.Values))
	}
