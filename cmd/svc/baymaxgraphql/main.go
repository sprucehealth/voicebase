...
var (
	...
	flagIntercomSecretKey = flag.String("intercom_secret_key", "", "secret key for intercom secure mode")
...
func main() {
	...
	*flagStaticURLPrefix = strings.Replace(*flagStaticURLPrefix, "{BuildNumber}", boot.BuildNumber, -1)
	...
	baymaxgraphqlsettings.ScheduledMessagesConfig.GetBoolean().Default.Value = !environment.IsProd()
