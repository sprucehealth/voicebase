...
func (w *Worker) createTrainingCaseSet() error {
	...
	for _, trainingCase := range trainingCases {
		...
		if err := w.dataAPI.UpdatePatient(res.Patient.ID, update, false); err != nil {
			return err
		}
		...
		err = w.dataAPI.UpdatePatientPharmacy(trainingCase.PatientToCreate.ID, trainingCase.PatientToCreate.Pharmacy)
		if err != nil {
			return err
		}
