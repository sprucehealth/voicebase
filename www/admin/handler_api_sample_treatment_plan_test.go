...
func TestSTPHandlerGETSuccess(t *testing.T) {
	...
	test.OK(t, err)
	stp := []byte(`{"yo":"datums"}`)
	handler := newSampleTreatmentPlanHandler(mockedDataAPI_stpHandler{stp: stp})
...
func TestSTPHandlerGETSuccessNoRecord(t *testing.T) {
	r, err := http.NewRequest("GET", "mock.api.request?pathway_tag=foo", nil)
	test.OK(t, err)
	handler := newSampleTreatmentPlanHandler(mockedDataAPI_stpHandler{pathwayErr: api.ErrNotFound("Not found")})
...
func TestSTPHandlerPUTSuccess(t *testing.T) {
	...
	test.OK(t, err)
	handler := newSampleTreatmentPlanHandler(mockedDataAPI_stpHandler{pathwayTag: "foo", stp: []byte(`{"yo":"datums"}`), t: t})
	var response interface{}
	expectedWriter, responseWriter := httptest.NewRecorder(), httptest.NewRecorder()
	httputil.JSONResponse(expectedWriter, http.StatusOK, response)
	handler.ServeHTTP(context.Background(), responseWriter, r)
