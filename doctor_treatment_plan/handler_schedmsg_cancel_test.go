...
func (m *mockDataAPI_schedmsg) CancelTreatmentPlanScheduledMessage(messageID int64) (bool, error) {
	defer m.Record(messageID)
...
func TestCancelSchedMsg_alreadyCancelled(t *testing.T) {
	treatmentPlanID := uint64(10)
	doctorID := uint64(20)
	messageID := int64(30)
	caseID := uint64(40)
...
func TestCancelSchedmsg_TPActive(t *testing.T) {
	...
	m.Expect(mock.NewExpectation(m.GetDoctorIDFromAccountID, accountID))
	m.Expect(mock.NewExpectation(m.GetAbridgedTreatmentPlan, int64(treatmentPlanID), int64(0)))
	m.Expect(mock.NewExpectation(m.CancelTreatmentPlanScheduledMessage, messageID))
