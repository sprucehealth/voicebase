...
func TestPatientVisitsList_Patient(t *testing.T) {
	...
	patientVisit, err := testData.DataAPI.GetPatientVisitForSKU(patient.ID, test_integration.SKUAcneFollowup)
	test.OK(t, err)

	test_integration.SubmitPatientVisitForPatient(patient.ID, patientVisit.ID.Int64(), testData, t)
...
func TestQueryingSubmittedVisits(t *testing.T) {
	...
	visits, err := testData.DataAPI.VisitsSubmittedForPatientSince(patient.ID, time.Now().Add(10*time.Minute))
	test.OK(t, err)
