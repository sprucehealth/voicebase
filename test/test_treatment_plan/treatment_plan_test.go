...
func TestTreatmentPlanViews(t *testing.T) {
	...
	pcli := test_integration.PatientClient(testData, t, patient.ID)
...
func TestTreatmentPlanList_DiffTPStates(t *testing.T) {
	...
	treatmentPlanResponse := test_integration.GetListOfTreatmentPlansForPatient(patient.ID, doctor.AccountID.Int64(), testData, t)
...
func TestTreatmentPlan_MultipleCases(t *testing.T) {
	...
	cases, err := dc.CasesForPatient(patient.ID)
	test.OK(t, err)
