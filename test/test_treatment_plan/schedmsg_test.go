...
func TestScheduledMessage(t *testing.T) {
	...
	test.Equals(t, false, msgs[0].DeprecatedScheduledFor.IsZero())
	test.Equals(t, msgs[0].DeprecatedScheduledFor.Unix(), msgs[0].ScheduledForEpoch)
...
func TestScheduledMessageSend(t *testing.T) {
	...
	msgs, err := testData.DataAPI.ListCaseMessages(tp.PatientCaseID.Int64(), api.LCMOIncludePrivate)
	test.OK(t, err)
	test.Equals(t, 1, len(msgs))
