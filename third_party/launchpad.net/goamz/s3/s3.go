...
func (b *Bucket) Put(path string, data []byte, contType string, perm ACL, additionalHeaders map[string][]string) error {
	body := bytes.NewBuffer(data)
	return b.PutReader(path, body, int64(len(data)), contType, perm, additionalHeaders)
}

// PutReader inserts an object into the S3 bucket by consuming data
// from r until EOF.
func (b *Bucket) PutReader(path string, r io.Reader, length int64, contType string, perm ACL, additionalHeaders map[string][]string) error {
	headers := map[string][]string{
...
func (b *Bucket) SignedURL(path string, expires time.Time, additionalHeaders map[string][]string) string {
	...
	if b.S3.Auth.Token != "" {
		return u.String() + "&x-amz-security-token=" + url.QueryEscape(b.S3.Auth.Token)
	} else {
		return u.String()
	}
}
