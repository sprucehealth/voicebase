...
module.exports = {
	CareProvidersPage: React.createClass({displayName: "CareProvidersPage",
		...
		componentWillMount: function() {
			if (!this.props.page) {
				this.props.router.navigate("/careproviders/search", {replace: true});
			}
		},
		render: function(): any {
			if (!this.props.page) {
	...
	Doctor: React.createClass({displayName: "Doctor",
		...
		onAccountUpdate: function(account: any): void {
			...
			return (
				<div>
					<Nav.LeftNav router={this.props.router} items={this.menuItems} currentPage={this.props.page}>
						...
						{this.state.doctor == null ? <Utils.LoadingAnimation /> : this.pages[this.props.page].bind(this)()}
					</Nav.LeftNav>
				</div>
			);
		}
...
var DoctorSearch = React.createClass({displayName: "DoctorSearch",
	...
	search: function(q) {
		this.props.router.navigate("/careproviders/search?q=" + encodeURIComponent(q), {replace: true}); // TODO: replacing until back tracking works
		if (q == "") {
...
var DoctorSearchResult = React.createClass({displayName: "DoctorSearchResult",
	...
	render: function(): any {
		return (
			<a href={"/careproviders/account/"+this.props.result.doctor_id+"/info"} onClick={this.onNavigate}>{"Dr. "+this.props.result.first_name+" "+this.props.result.last_name+" ("+this.props.result.email+")"}</a>
		);
	}
});
...
var DoctorInfoPage = React.createClass({displayName: "DoctorInfoPage",
	...
	onTwoFactorToggle: function(e: Event) {
		e.preventDefault();
		...
		AdminAPI.careProviderEligibility(this.props.doctor.id, function(success, data, error) {
			if (success) {
				if (this.isMounted()) {
					...
					this.sortMappings(mappings, this.state.pathwayMap);
					this.setState({mappings: mappings, busy: false});
				}
			} else {
				this.setState({error: error.message});
			}
...
var CareProviderStatePathwayMappings = React.createClass({displayName: "CareProviderStatePathwayMappings",
	mixins: [Routing.RouterNavigateMixin],
	getInitialState: function(): any {
		return {
			error: null,
	...
	handleChangeState: function(e: any): void {
		e.preventDefault();
		this.setState({stateCode: e.target.value});
	},
	handleChangePathway: function(e: any): void {
		e.preventDefault();
...
var StatePathwayMappingsSummary = React.createClass({displayName: "StatePathwayMappingsSummary",
	...
	propTypes: {
		...
		onClickState: React.PropTypes.func.isRequired,
		onClickPathway: React.PropTypes.func.isRequired
	},
					...
					{this.state.mappings.map(function(m) {
						...
						return (
							<tr key={"mapping-" + m.id}>
								...
								<td>
									<a href={"/careproviders/account/" + m.provider.id + "/eligibility"} onClick={this.onNavigate}>
