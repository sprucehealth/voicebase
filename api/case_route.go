...
func (d *DataService) ExtendClaimForDoctor(doctorID, patientID, patientCaseID int64, duration time.Duration) error {
	...
	_, err = tx.Exec(`
		...
		SET expires = ?
		WHERE provider_id = ? AND role_type_id = ? AND status = ? AND patient_case_id = ?`,
		expires, doctorID, d.roleTypeMapping[RoleDoctor],
		StatusTemp, patientCaseID)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func (d *DataService) TransitionToPermanentAssignmentOfDoctorToCaseAndPatient(doctorID int64, patientCase *common.PatientCase) error {
	...
	// first check to ensure that the doctor is assigned to patient case and file
	var count int64
	if err := tx.QueryRow(`
		SELECT count(*)
		FROM patient_care_provider_assignment
	...
	// update patient case to indicate that is claimed
	_, err = tx.Exec(`
		UPDATE patient_case
		SET claimed = 1
		WHERE id = ?`, patientCase.ID.Int64())
	...
	_, err = tx.Exec(`
		...
		WHERE patient_case_id = ? AND doctor_id = ? AND locked = ?`, patientCase.ID.Int64(), doctorID, true)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func (d *DataService) GetTempClaimedCaseInQueue(patientCaseID int64) (*DoctorQueueItem, error) {
	...
	err := d.db.QueryRow(`
		...
		FROM unclaimed_case_queue
		WHERE locked = ? AND patient_case_id = ?`,
		true, patientCaseID,
	).Scan(
		&queueItem.ID,
		&queueItem.EventType,
...
func (d *DataService) RevokeDoctorAccessToCase(patientCaseID, patientID, doctorID int64) error {
	...
	_, err = tx.Exec(`
		DELETE FROM patient_case_care_provider_assignment
		WHERE provider_id = ? AND role_type_id = ? AND status = ? AND patient_case_id = ?`,
