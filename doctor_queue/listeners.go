...
func InitListeners(dataAPI api.DataAPI, analyticsLogger analytics.Logger, dispatcher *dispatch.Dispatcher,
	...
	dispatcher.Subscribe(func(ev *app_event.AppEvent) error {
		...
		if ev.Resource == "all_case_messages" && ev.Action == app_event.ViewedAction {
			...
			maID, err := dataAPI.GetDoctorIDFromAccountID(ev.AccountID)
			if err != nil {
				golog.Errorf("Unable to get doctor id from account id: %s", err)
				return err
			}

			patientCase, err := dataAPI.GetPatientCaseFromID(ev.ResourceID)
			if err != nil {
				golog.Errorf("Unable to get case from id: %s", err)
				return err
			}

			careTeams, err := dataAPI.CaseCareTeams([]int64{patientCase.ID.Int64()})
			if err != nil {
				golog.Errorf("Unable to get care team for case: %s", err)
				return err
			} else if len(careTeams) != 1 {
				return fmt.Errorf("Expected single care team for case but got %d", len(careTeams))
			}

			var doctorAssignment, maAssignment *common.CareProviderAssignment
			for _, member := range careTeams[patientCase.ID.Int64()].Assignments {
				if member.Status != api.StatusActive {
					continue
				}

				switch member.ProviderRole {
				case api.RoleDoctor:
					doctorAssignment = member
				case api.RoleMA:
					maAssignment = member
				}
			}
			...
			if err := dataAPI.UpdateDoctorQueue([]*api.DoctorQueueUpdate{
				{
					...
					QueueItem: &api.DoctorQueueItem{
						...
						ItemID:           patientCase.ID.Int64(),
						EventType:        api.DQEventTypeCaseMessage,
						Status:           api.DQItemStatusReplied,
						Description:      fmt.Sprintf("Message from patient"),
						ShortDescription: fmt.Sprintf("Message from patient"),
						ActionURL:        app_url.ViewPatientMessagesAction(patientCase.PatientID.Int64(), patientCase.ID.Int64()),
						Tags:             []string{patientCase.Name},
					},
				},
			}); err != nil {
				...
				if err := dataAPI.UpdateDoctorQueue([]*api.DoctorQueueUpdate{
					{
						...
						QueueItem: &api.DoctorQueueItem{
							...
							EventType:        api.DQEventTypeCaseAssignment,
							Status:           api.DQItemStatusReplied,
							Description:      fmt.Sprintf("Case assigned by %s", doctorAssignment.ShortDisplayName),
							ShortDescription: fmt.Sprintf("Case assigned by %s", doctorAssignment.ShortDisplayName),
							ActionURL:        app_url.ViewPatientMessagesAction(patientCase.PatientID.Int64(), patientCase.ID.Int64()),
							Tags:             []string{patientCase.Name},
						},
					},
			...
			if doctorAssignment != nil && maAssignment != nil {
				// clear out any case assignments in the doctor's queue pertaining to this case
				if err := dataAPI.UpdateDoctorQueue([]*api.DoctorQueueUpdate{
					{
						Action:       api.DQActionReplace,
						CurrentState: api.DQItemStatusPending,
						QueueItem: &api.DoctorQueueItem{
							DoctorID:         doctorAssignment.ProviderID,
							PatientID:        patientCase.PatientID.Int64(),
							ItemID:           patientCase.ID.Int64(),
							EventType:        api.DQEventTypeCaseAssignment,
							Status:           api.DQItemStatusReplied,
							Description:      fmt.Sprintf("Case assigned by %s", maAssignment.ShortDisplayName),
							ShortDescription: fmt.Sprintf("Case assigned by %s", maAssignment.ShortDisplayName),
							ActionURL:        app_url.ViewPatientMessagesAction(patientCase.PatientID.Int64(), patientCase.ID.Int64()),
							Tags:             []string{patientCase.Name},
						},
					},
