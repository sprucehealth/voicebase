...
func NewDiscoverer(discoveryHost string, updateInterval time.Duration) (*Discoverer, error) {
	if strings.IndexByte(discoveryHost, ':') < 0 {
		discoveryHost += ":11211"
	}
	hosts, ver, err := Discover(discoveryHost, "")
	if err != nil {
		return nil, err
	}
	d := &Discoverer{
...
func (d *Discoverer) Unsubscribe(ch chan<- []string) {
	d.smu.Lock()
	delete(d.subscribers, ch)
	d.smu.Unlock()
}

func (d *Discoverer) Hosts() []string {
	d.mu.RLock()
	hosts := d.hosts
	d.mu.RUnlock()
	return hosts
...
func (d *Discoverer) loop(interval time.Duration) {
	...
	for {
		...
		hosts, ver, err := Discover(d.dhost, d.ver)
		if err == ErrNoChange {
			continue
		} else if err != nil {
...
// Discover returns the list of memcached nodes. prevVer is the
...
func Discover(host string, prevVer string) ([]string, string, error) {
